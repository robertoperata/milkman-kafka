...
class RecordBuilder {
    ...
    static List<HadoopRecord> getHadoopRecords(PosInvoice invoice) {
        ...
        for (LineItem i : invoice.getInvoiceLineItems()) {
            ...
            if (invoice.getDeliveryType().toString().equalsIgnoreCase(AppConfig.DELIVERY_TYPE_HOME_DELIVERY)) {
    ...
    static Notification getNotification(PosInvoice invoice) {
        ...
        notification.setCustomerCardNo(invoice.getCustomerCarNo().toString());
        notification.setTotalAmount(invoice.getTotalAmount());
        notification.setEarnedLoyaltyPoints(invoice.getTotalAmount() * AppConfig.LOYALTY_FACTOR);
        return notification;
    }
}
